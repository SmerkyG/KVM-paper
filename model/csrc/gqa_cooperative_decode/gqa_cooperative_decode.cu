...
template <int RouteSplits, bool AdaptiveSplits, bool Int8Storage>
__global__ __launch_bounds__(256) void gqa_cooperative_decode_kernel(
    const bf16* __restrict__ q,
    ...
    int cache_batch = static_cast<int>(cache_indices[batch]);
    ...
    __shared__ float shared_probabilities[kGqaHeads * kPageSize];
    __shared__ int8_t shared_probability_codes[kGqaHeads * kPageSize];
    __shared__ float shared_correction[kGqaHeads];
    __shared__ float shared_maximum[kGqaHeads];
    __shared__ float shared_denominator[kGqaHeads];
    __shared__ int shared_slot;
    __shared__ int shared_key_count;
    __shared__ int shared_selected_mask;
    __shared__ int shared_selected_rank[kGqaHeads];
    __shared__ int shared_leader;

    if (tid == 0) {
        const int candidate_head = candidate / kRouteCount;
        const int candidate_rank = candidate % kRouteCount;
        const int query_head_base = kv_head * kGqaHeads;
        const int64_t slot = top_slots[
            static_cast<int64_t>(batch) * top_batch_stride
        ...
        for (int earlier = 0; earlier < candidate && leader; ++earlier) {
            const int earlier_head = earlier / kRouteCount;
        ...
        int selected_mask = 0;
        for (int selected_head = 0; selected_head < kGqaHeads; ++selected_head) {
            shared_selected_rank[selected_head] = 0;
            for (int rank = 0; rank < kRouteCount; ++rank) {
                ...
                if (selected_slot == slot && slot >= 0) {
                    selected_mask |= 1 << selected_head;
                    shared_selected_rank[selected_head] = rank;
                }
            }
        }
    ...
    const int query_row = batch * query_heads + kv_head * kGqaHeads + head;
    float query_fragment[4];
    int packed_query = 0;
    ...
    for (
        ...
        __syncthreads();
        for (int index = tid; index < kPageSize * kHeadDim; index += 256) {
...
extern "C" int launch_gqa_cooperative_decode(
    ...
    float scale_log2,
    void* stream) {
    if (query_heads != kv_heads * kGqaHeads) {
    ...
    const dim3 grid(
        ...
        static_cast<unsigned int>(kGqaHeads * kRouteCount),
        static_cast<unsigned int>(route_splits));
#define LAUNCH_GQA_COOPERATIVE(RouteSplits, AdaptiveSplits, Int8Storage)     \
    hipLaunchKernelGGL(                                                       \
        (gqa_cooperative_decode_kernel<                                      \
            RouteSplits, AdaptiveSplits, Int8Storage>),                      \
        grid,                                                                 \
        dim3(256),                                                            \
        0,                                                                    \
        hip_stream,                                                           \
    ...
    if (adaptive_splits && int8_storage) {
        switch (route_splits) {
            case 8:
                LAUNCH_GQA_COOPERATIVE(8, true, true);
                break;
            case 16:
                LAUNCH_GQA_COOPERATIVE(16, true, true);
                break;
            case 32:
                LAUNCH_GQA_COOPERATIVE(32, true, true);
                break;
            default:
                return static_cast<int>(hipErrorInvalidValue);
        }
        ...
        switch (route_splits) {
            case 8:
                LAUNCH_GQA_COOPERATIVE(8, true, false);
                break;
            case 16:
                LAUNCH_GQA_COOPERATIVE(16, true, false);
                break;
            case 32:
                LAUNCH_GQA_COOPERATIVE(32, true, false);
                break;
            default:
                return static_cast<int>(hipErrorInvalidValue);
        }
        ...
        switch (route_splits) {
            case 4:
                LAUNCH_GQA_COOPERATIVE(4, false, true);
                break;
            case 8:
                LAUNCH_GQA_COOPERATIVE(8, false, true);
                break;
            case 16:
                LAUNCH_GQA_COOPERATIVE(16, false, true);
                break;
            case 32:
                LAUNCH_GQA_COOPERATIVE(32, false, true);
                break;
        }
    } else {
        switch (route_splits) {
            case 4:
                LAUNCH_GQA_COOPERATIVE(4, false, false);
                break;
            case 8:
                LAUNCH_GQA_COOPERATIVE(8, false, false);
                break;
            case 16:
                LAUNCH_GQA_COOPERATIVE(16, false, false);
                break;
            case 32:
                LAUNCH_GQA_COOPERATIVE(32, false, false);
                break;
        }
    }
