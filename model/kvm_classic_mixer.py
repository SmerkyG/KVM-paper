...
class SequenceMixer(TorchKVMSequenceMixer):
    ...
    - state updates use rounded fp16 deltas and cached normalized state;
    ...
    def __init__(self, config, layer_idx: int):
        ...
        if self.state_budget_mode == "kvm_saturation":
            raise ValueError(
                "kvm_classic_mixer currently supports fixed and power_law state schedules only"
            )
    ...
    def _triton_prefill_out(
        ...
        if not gates_enabled:
            raise ValueError("kvm_classic_mixer training assumes merge gates exist")
    ...
    def forward_prefill(
        ...
        **kwargs,
        ...
        if not gates_enabled:
            raise ValueError("kvm_classic_mixer prefill assumes merge gates exist")
    ...
    def _update_state_from_overflow_tokens(
        ...
        if not overflow_k.is_cuda:
            raise RuntimeError("kvm_classic_mixer generation updates require CUDA/ROCm")
