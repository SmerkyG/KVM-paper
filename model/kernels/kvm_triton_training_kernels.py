"""Current integrated KVM Triton training kernels.

This module contains the active prefill, periodic state-update, and backward
kernels used by ``model.kvm_triton_mixer``. The code was originally developed
in benchmark/prototype files; keep model integration imports pointed here so
benchmark-only variants do not become part of the production path by accident.
"""
...
def _grouped_scan_oldstate_maxsim_kernel(
    ...
    for local_chunk in tl.range(0, GROUP_CHUNKS, 1, num_stages=1):
        ...
        rel_candidates = tl.where(
            merge_scores == local_best_score[:, None], state_offsets[None, :], 0
        )
        local_best_rel = tl.max(rel_candidates, axis=1)
...
@triton.jit
def _reduce_oldstate_maxsim_quota_append_kernel(
    state_k,
    state_v,
    state_k_attn,
    state_v_attn,
    ...
    row = tl.program_id(0).to(tl.int64)
    sub_id = tl.program_id(1).to(tl.int64)
    token_offsets = tl.arange(0, SUB_BLOCK)
    key_offsets = tl.arange(0, HEAD_DIM)
    ...
    select_best_score = tl.full((SUB_BLOCK,), -float("inf"), tl.float32)
    merge_best_score = tl.full((SUB_BLOCK,), -float("inf"), tl.float32)
    merge_best_idx = tl.full((SUB_BLOCK,), -1, tl.int32)
    ...
    for group_id in tl.range(0, MAX_STATE_GROUPS, 1, num_stages=1):
        ...
        merge_score = tl.load(partial_row + offsets)
        merge_idx = tl.load(partial_idx_row + offsets)
        select_best_score = tl.maximum(select_best_score, select_score)
        take_merge = merge_score > merge_best_score
        merge_best_score = tl.where(take_merge, merge_score, merge_best_score)
        merge_best_idx = tl.where(take_merge, merge_idx, merge_best_idx)
    ...
    base_quota = n_append // SUB_BLOCKS
    remainder = n_append - base_quota * SUB_BLOCKS
    sub_id_i32 = sub_id.to(tl.int32)
    append_quota = base_quota + tl.where(sub_id_i32 < remainder, 1, 0)
    append_base = sub_id_i32 * base_quota + tl.minimum(sub_id_i32, remainder)
    append_token = score_rank < append_quota

    append_before = append_token[None, :] & (
        token_offsets[None, :] < token_offsets[:, None]
    )
    append_rank = tl.sum(append_before.to(tl.int32), axis=1)
    append_pos = state_before + append_base + append_rank
    tl.store(
        append_pos_row + macro_id * MACRO_BLOCK + macro_token,
        tl.where(append_token, append_pos, -1),
    )
    tl.store(
        best_idx_row + macro_id * MACRO_BLOCK + macro_token,
        tl.where(append_token, -1, merge_best_idx),
    )
...
def _token_fp16_delta_update_updated_state_store_bestidx_kernel(
    ...
    if SCAN_APPEND_TARGETS:
        for append_sub_id in tl.range(0, SUB_BLOCKS, 1, num_stages=1):
            ...
            local_best_idx = tl.max(
                tl.where(scores == local_best_score[:, None], candidate_pos[None, :], -1),
                axis=1,
            ).to(tl.int32)
            take_local = local_best_score > best_score
            best_score = tl.where(take_local, local_best_score, best_score)
...
def run_forward_state_update(
    ...
    *,
    ...
    if append_policy != "subblock_quota":
        raise ValueError("kvm_triton_training_kernels only supports subblock_quota")
    if merge_order != "merge_before_append":
        raise ValueError("kvm_triton_training_kernels only supports merge_before_append")
    ...
    _grouped_scan_oldstate_maxsim_kernel[
        ...
        SPLIT_SELECT_MERGE=False,
        **triton_launch_kwargs(
            args.scan_num_warps, 1, args.scan_waves_per_eu or args.waves_per_eu
        ),
    )
    _reduce_oldstate_maxsim_quota_append_kernel[(kv_rows, sub_blocks)](
        state_k,
        state_v,
        state_k_attn,
        state_v_attn,
...
__all__ = [
    ...
    "_forward_apply_fp16_delta_normcache",
    "_grouped_scan_oldstate_maxsim_kernel",
    "_reduce_oldstate_maxsim_quota_append_kernel",
    "_token_fp16_delta_update_updated_state_store_bestidx_kernel",
