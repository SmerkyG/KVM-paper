"""Inference-only top-k LOD attention graft for Qwen3.5.

Qwen3.5 interleaves Gated DeltaNet and ordinary softmax-attention layers.  This
module changes only the latter.  Old softmax KV leaves are partitioned into a
16*sqrt(T) state; a query expands the leaves of its top-k state slots and uses
count-corrected mean KV summaries for every other slot.  The exact and coarse
branches are combined with their log-sum-exp statistics.

The exact-leaf archive is stored in 16-token pages, while decode keeps only a
bounded recent KV window. This preserves the original weights while avoiding a
second full contiguous KV cache.
"""
...
from .kernels.qwen35_lod_kernels import (
...
class Qwen3_5TwoLevelAttention(Qwen3_5Attention):
    """Qwen3.5 full-attention layer with a mass-corrected top-k LOD remote."""
    ...
    # Dense BF16 state aggregation is faster for batch-one Qwen inference;
    ...
    def _coarse_attention(
        ...
        *,
        ...
        if actual_local_len > local_capacity:
            raise AssertionError("Qwen3.5 LOD local window exceeded its capacity")
        ...
        if state_len > state_capacity:
            raise AssertionError("Qwen3.5 LOD state exceeded padded capacity")
    ...
    def _two_level_attention(
        ...
        *,
        ...
        if top_slots is None:
            raise AssertionError("Qwen3.5 LOD routing did not produce slots")
    ...
    def _prefill_attention(
        ...
        for query_begin in range(front_len, prefill_len, prefill_chunk_len):
            ...
            if state_coverage != bswa_begin:
                raise AssertionError("Qwen3.5 LOD prefill state coverage drifted")
            outputs.append(
    ...
    def _decode_attention(
        ...
        *,
        ...
        if not hasattr(self, "_lod_state"):
            raise RuntimeError("Qwen3.5 LOD decode did not receive a prefill state")
        ...
        if not isinstance(recent_k, torch.Tensor) or not isinstance(
            ...
            raise TypeError("Qwen3.5 LOD recent cache is missing")
        previous_total_len = int(cache["total_len"])
        if total_len != previous_total_len + int(new_k.size(2)):
            raise AssertionError("Qwen3.5 LOD decode position drifted")
        ...
        if state_coverage < target_coverage:
            ...
            if page_cache is None:
                ...
                if remaining < 0:
                    raise AssertionError("Qwen3.5 LOD local cache underflowed")
                if remaining:
        ...
        if new_bswa_begin == 0:
            ...
            if page_cache is None:
                ...
                if recent_len >= int(recent_k.size(2)):
                    raise AssertionError("Qwen3.5 LOD local cache overflowed")
                local_k = recent_k
                local_v = recent_v
                append_k = new_k
                append_v = new_v
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (
            ...
            # The exact historical K/V already lives in the leaf page cache.
            # Keep only a zero-storage tensor whose shape lets Qwen's hybrid
            # cache report the correct sequence length to generation code.
            metadata_shape = (int(key_states.size(0)), 0, total_len, 0)
            metadata_k = past_key_values.key_cache[self.layer_idx]
            metadata_v = past_key_values.value_cache[self.layer_idx]
            if metadata_k is None:
                metadata_k = key_states.new_empty(metadata_shape)
                metadata_v = value_states.new_empty(metadata_shape)
                past_key_values.key_cache[self.layer_idx] = metadata_k
                past_key_values.value_cache[self.layer_idx] = metadata_v
            else:
                metadata_k.resize_(metadata_shape)
                if metadata_v is None:
                    raise AssertionError("Qwen3.5 LOD metadata V cache is missing")
                metadata_v.resize_(metadata_shape)
        if is_prefill:
    ...
    *,
    ...
    for layer_idx, layer in enumerate(text_model.layers):
        ...
        if not isinstance(attention, Qwen3_5Attention):
            raise TypeError(
                f"layer {layer_idx} attention has unexpected type {type(attention)!r}"
            )
