...
"""Qwen3.8 DFlash2 support for the vLLM revision used by this release."""
...
@triton.jit
def _prepare_dflash2_inputs_kernel(
    out_input_ids_ptr,
...
def _requests_dflash2(vllm_config: VllmConfig) -> bool:
...
def _is_dflash2(vllm_config: VllmConfig) -> bool:
    if not _requests_dflash2(vllm_config):
        return False
    target = vllm_config.model_config.hf_config
    text = getattr(target, "text_config", None) or target
    return (
        str(getattr(target, "model_type", "")) == "qwen3_5"
        and int(getattr(text, "num_attention_heads", 0)) == 24
        and int(getattr(text, "num_key_value_heads", 0)) == 4
        and int(getattr(text, "head_dim", 0)) == 256
    )
...
def register_dflash2_compat() -> None:
    ...
    ModelRegistry.register_model(
        "DFlash2DraftModel",
        "vllm_lod_plugin.models.dflash2:DFlash2Qwen3ForCausalLM",
    )
    ...
    dflash_speculator._prepare_dflash_inputs_kernel = _prepare_dflash2_inputs_kernel
    ...
    def init_speculator(vllm_config: VllmConfig, device: torch.device):
        ...
        if _requests_dflash2(vllm_config):
